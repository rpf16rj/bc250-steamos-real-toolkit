...
- **Fixed:** the AIC8800 driver's vendor `Makefile` (`steamos-headers` target) has the same "jupiter-main" repo-channel problem as the audio fix, plus its own bug: it derives `pkgver` with a single-hyphen-to-dot substitution (`sed -e 's/-/./'`, no `g` flag), which mishandles kernel flavors whose version string has more than one hyphen (e.g. `-drm-exec`'s `drmexec7-valve24.3` needs *both* hyphens converted, not just the first) — it either 404s or requests the wrong filename. Refactored the header-package logic shared with the audio fix into `bc250_headers_pkg_name()`/`bc250_fetch_headers_pkg()`, and added `aic8800_prefetch_headers()`, which pre-extracts the correct package straight into the vendor tree's expected `steamos-headers/` directory — `steamdeck-setup.sh` only invokes the broken target when that directory doesn't already exist, so this sidesteps it with zero changes to the vendor Makefile.
- Validated the full AIC8800 WiFi/BT driver install end-to-end on real BC-250 hardware (custom `drm-exec` kernel flavor): build, module install, `/etc` config, dongle mode-switch, `wlan0` up.
